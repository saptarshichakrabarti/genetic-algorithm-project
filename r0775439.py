...
class r0775439:
	...
	def optimize(self, filename):
		...
		# --> 0.75 to 1.0 instead of 0.5 to 1.0 to favour greedier solution mix
		greedy_mix_percentage = 0.5 if num_cities < 100 else ((num_cities - 50) * (1.0 - 0.75) / (1000 - 50)) + 0.75
		
		crossover_probability = 0.95
		mutation_probability = 0.95  # Initial mutation probability
		...
		# Parematers for fitness sharing
		sigma = np.linalg.norm(np.min(population_fitness) / 1000)
		...
		# Your code here.
		yourConvergenceTestsHere = generation < iterations
		...
		while( yourConvergenceTestsHere ):
			...
			# Mutation
			children = mutate(population, children, mutation_population_percentage, mutation_probability, distance_matrix)
			# check_population_validity(children)
			...
			sigma = np.linalg.norm(population_history[-1][4] / 1000)
			alpha = alpha * 0.9999


			meanObjective = population_history[-1][3]
			bestObjective = population_history[-1][4]
			bestSolution = population_history[-1][5]
			...
			if generation > 100 and population_history[-1][3] == population_history[-50][3]:
				break
...
def initiate_population(distance_matrix: np.array, population_size: np.int32,
	...
	if greedy:
		...
		# Determine the size of the mixed population and handle larger than 1000 cities cases
...
def mutate(pop, children, population_percentage, mutation_probability, distance_matrix):

	population = np.vstack((pop, children))
	...
	# Keep only those children that are not already in the population
	mutated_children = np.array([child for child in mutated_children if not any(np.array_equal(child, ind) for ind in pop)])

	if len(mutated_children) != 0:
		return np.vstack((children, mutated_children))
	else:
		return children
